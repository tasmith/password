...
*** Slowing Down Brute Force
    ...
    every use has a different salt precomputed rainbow tables won't help because
    ...
    limit that a use will wait to decrypt a secure email.  Hardware that runs
    these hashes a million times faster will still be able to attempt 
    hundreds of thousands of passwords per second.
